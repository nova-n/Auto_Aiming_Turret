...
if webCamCapture.isOpened():
    ...
    res = "<X{fW},Y{fH}>".format(fW=vidWidth,fH=vidHeight) #remember, the resolution is sent as coordinates
...
#waits for the serial buffer to be clear of any characters
while arduino.in_waiting == 0:
	pass #just wait for input
print("past wait")#prints once the serial buffer is empty
while True:
	#once the serial buffer is empty, the arduino can finally send the message 'Buffer Cleared, Ready To Start!'
	#with no other characters before it
	#this means that the raspberry pi knows FOR SURE that the rest of the program is ready to start
...
#the arduino will send back the resolution string, so you can see it in the terminal
